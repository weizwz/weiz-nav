...
import { performDebouncedSearch } from '@weiz-nav/store/slices/searchSlice';
import { setSearchEngine } from '@weiz-nav/store/slices/settingsSlice';
import { SEARCH_ENGINES, getSearchUrl, getSearchEngine } from '@weiz-nav/core/search';
...
export default function SearchBar() {
   ...
   * 获取搜索引擎图标
   */
  const getEngineIcon = React.useCallback(
    (iconUrl: string) => {
  ...
  const handleKeyPress = React.useCallback(
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.key === 'Enter') {
        ...
        setInputValue('');
        dispatch(performDebouncedSearch(''));
      }
    },
    [handleSearch, inputValue, dispatch]
  );
